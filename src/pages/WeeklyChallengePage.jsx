...
export default function WeeklyChallengePage() {
  ...
  const { challenges, currentChallengeStep, setChallengeStep, challengeAnswers, setChallengeAnswer,
    challengeDecision, setChallengeDecision, submitChallenge, challengeSubmitted, resetChallengeFlow } = useStore();
  ...
  if (!challenge) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center">
        ...
        <h2 className="font-[Quicksand] text-2xl font-bold text-gray-800 mb-2">Chưa có nhiệm vụ mới!</h2>
  ...
  if (challengeSubmitted) {
    return (
      <>
        <Confetti active={true} pieces={50} duration={4000} />
        <motion.div className="flex flex-col items-center justify-center min-h-[60vh] text-center"
          ...
          <h2 className="font-[Quicksand] text-3xl font-bold text-gray-800 mb-3">Tuyệt vời, con giỏi lắm!</h2>
          <p className="text-gray-500 mb-2 max-w-md">
            Nhiệm vụ "{challenge.title}" đã được gửi cho phụ huynh xem xét.
          </p>
          <motion.div className="flex items-center gap-2 text-leaf-600 text-sm font-medium mb-6"
            initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.5, type: 'spring' }}>
            <Sparkles size={16} /> +{challenge.xpReward} XP
          </motion.div>
          <div className="flex gap-3">
            <button onClick={() => { resetChallengeFlow(); navigate('/child'); }}
              className="bg-gradient-to-r from-mint-500 to-leaf-500 text-white px-6 py-2.5 rounded-xl font-semibold hover:shadow-lg transition-all">
              Về trang chủ
            </button>
            <button onClick={() => { resetChallengeFlow(); navigate('/child/jars'); }}
  ...
  const nextStep = () => { if (step < STEPS.length - 1) setChallengeStep(step + 1); };
  const prevStep = () => { if (step > 0) setChallengeStep(step - 1); };
  const handleSubmit = () => submitChallenge();
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="mb-6">
        ...
        {/* Mini curriculum progress */}
        <div className="flex items-center gap-2 mb-3">
          <CurriculumTimeline challenges={challenges} compact />
          <span className="text-xs text-gray-400 ml-2">Tuần {challenge.week}/{challenges.length}</span>
        </div>
      ...
      <AnimatePresence mode="wait">
        <motion.div key={step} initial={{ opacity: 0, x: 30, scale: 0.98 }}
          animate={{ opacity: 1, x: 0, scale: 1 }} exit={{ opacity: 0, x: -30, scale: 0.98 }}
          transition={{ duration: 0.35, ease: 'easeOut' }}>

          {/* Step 0: Read */}
          {step === 0 && (
            <GlassCard className="mb-6">
              <motion.div className="text-5xl mb-4 text-center"
                animate={{ y: [0, -5, 0] }} transition={{ duration: 2, repeat: Infinity }}>📖</motion.div>
              <h3 className="font-semibold text-gray-800 text-lg mb-3 text-center">{challenge.title}</h3>
              <p className="text-gray-600 leading-relaxed text-center mb-6">{challenge.description}</p>
              ...
              {challenge.questions.map((q, idx) => (
                <GlassCard key={q.id}>
                  <div className="text-sm font-medium text-gray-700 mb-3">
                    <span className="text-mint-600 font-bold mr-1">Câu {idx + 1}.</span>{q.text}
                  </div>
                  {q.type === 'choice' && (
                    <div className="space-y-2">
                      {q.options.map((opt) => (
                        <motion.button key={opt} onClick={() => setChallengeAnswer(q.id, opt)}
                          whileTap={{ scale: 0.97 }}
                  ...
                  {q.type === 'open' && (
                    <textarea value={challengeAnswers[q.id] || ''} onChange={(e) => setChallengeAnswer(q.id, e.target.value)}
          ...
          {step === 2 && (
            <GlassCard>
              <h3 className="font-semibold text-gray-800 text-lg mb-2 text-center">Con quyết định thế nào?</h3>
              <p className="text-sm text-gray-500 text-center mb-6">Dựa trên những gì con đã học, hãy chọn hành động:</p>
              <div className="grid grid-cols-2 gap-3">
                {challenge.decisions.map((d) => {
                  const colorMap = { coral: 'bg-coral-50 border-coral-200 hover:bg-coral-100', sun: 'bg-sun-50 border-sun-200 hover:bg-sun-100', leaf: 'bg-leaf-50 border-leaf-200 hover:bg-leaf-100', mint: 'bg-mint-50 border-mint-200 hover:bg-mint-100' };
                  const selectedMap = { coral: 'bg-coral-500 border-coral-500 text-white', sun: 'bg-sun-500 border-sun-500 text-white', leaf: 'bg-leaf-500 border-leaf-500 text-white', mint: 'bg-mint-500 border-mint-500 text-white' };
                  const isSelected = challengeDecision === d.id;
                  return (
                    <motion.button key={d.id} onClick={() => setChallengeDecision(d.id)}
                      className={`border-2 rounded-2xl p-4 text-center transition-all duration-200 ${isSelected ? selectedMap[d.color] : colorMap[d.color]}`}
                      whileTap={{ scale: 0.93 }} animate={isSelected ? { scale: [1, 1.05, 1] } : {}}>
                      <motion.div className="text-3xl mb-2" animate={isSelected ? { rotate: [0, -10, 10, 0] } : {}}
                        transition={{ duration: 0.4 }}>{d.emoji}</motion.div>
                      <div className={`text-sm font-semibold ${isSelected ? 'text-white' : 'text-gray-700'}`}>{d.label}</div>
                    </motion.button>
                  );
                })}
              </div>
          ...
          {step === 3 && (
            <GlassCard>
              <div className="text-center">
                <motion.div className="text-5xl mb-4" animate={{ y: [0, -8, 0] }}
                  transition={{ duration: 2, repeat: Infinity }}>📨</motion.div>
                <h3 className="font-[Quicksand] text-xl font-bold text-gray-800 mb-2">Gửi cho phụ huynh xem nhé!</h3>
        ...
        <div className="flex justify-between mt-6">
          <button onClick={prevStep} disabled={step === 0}
            className={`flex items-center gap-2 px-5 py-2.5 rounded-xl font-medium text-sm transition-all ${
              step === 0 ? 'text-gray-300 cursor-not-allowed' : 'text-gray-600 hover:bg-white/60'
            }`}>
